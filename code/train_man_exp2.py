...
def train(train_sets, dev_sets, test_sets, unlabeled_sets):
    ...
    # 加载无label的验证数据以及测试数据
    # 原来有标签的2000个sample作为"无标签"的域使用，方便对比与单独训练有label时的情况
    # 而test_sets则是实实在在的raw_unlabeled_sets
    # 需要预测label的是test_loaders中的数据
    ...
    # 对于target domain unlabeled_sets也是原来有标签的2000个sample作为"无标签"的域使用
    # 而对于source domain则unlabeled_sets[domain] = raw_unlabeled_sets[domain]
