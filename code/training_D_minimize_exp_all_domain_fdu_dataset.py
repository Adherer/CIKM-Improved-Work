...
def t_sne(domain, n_components, visual_features, domain_labels):
    if n_components == 2:
        ...
        plt.savefig(domain + "_30_epoch_" + "T-SNE_Digits.png")
    elif n_components == 3:
        tsne_features = TSNE(n_components=n_components, random_state=35).fit_transform(visual_features)
        aim_data = plot_embedding(tsne_features)
        fig = plt.figure()
        ...
        plt.title("T-SNE Digits")
        plt.savefig(domain + "_30_epoch_" + "T-SNE_Digits_3d.png")
    else:
...
def main():
    ...
    unlabeled_domains = ['MR', 'books', 'magazines']
    test_acc_dict = {}
    ave_test_acc = 0.0
    for domain in unlabeled_domains:
        opt.domains = ['MR', 'apparel', 'baby', 'books', 'camera_photo', 'dvd', 'electronics',
                        'health_personal_care', 'imdb', 'kitchen_housewares', 'magazines', 'music',
        ...
        cv, visual_features, domain_labels = train(vocab, train_sets, dev_sets, test_sets, unlabeled_sets)
