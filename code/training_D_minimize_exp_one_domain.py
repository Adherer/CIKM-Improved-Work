        ...
        for i in tqdm(range(num_iter)):
            ...
            for domain in opt.all_domains:
                ...
                if opt.loss.lower() == 'gr':
                    ...
                    private_l_d = 0.0
                    if domain != opt.dev_domains[0]:
                        private_l_d = functional.nll_loss(private_d_outputs, d_targets) * -1.       # 注意这边的loss function
                    l_d_sum = shared_l_d + private_l_d
                    if opt.lambd > 0:
                        l_d_sum *= -opt.lambd
                    ...
                    private_l_d = 0.0
                    if domain != opt.dev_domains[0]:
                        private_l_d = functional.kl_div(private_d_outputs, d_targets, size_average=False) * -1.
                    l_d_sum = shared_l_d + private_l_d
                    if opt.lambd > 0:
                        l_d_sum *= opt.lambd
                    ...
                    private_l_d = 0.0
                    if domain != opt.dev_domains[0]:
                        private_l_d = functional.mse_loss(private_d_outputs, d_targets) * -1.
                    l_d_sum = shared_l_d + private_l_d
                    if opt.lambd > 0:
                        l_d_sum *= opt.lambd
                l_d_sum.backward()
...
def t_sne(n_components, visual_features, domain_labels):
    if n_components == 2:
...
def main():
    ...
    t0 = time()
    t_sne(2, visual_features.detach().cpu().numpy(), domain_labels.detach().cpu().numpy())
    print("t-SNE 2D embedding of the digits (time %.2fs)" % (time() - t0))

    print("Computing t-SNE 3D embedding")
    t0 = time()
    t_sne(3, visual_features.detach().cpu().numpy(), domain_labels.detach().cpu().numpy())
    print("t-SNE 2D embedding of the digits (time %.2fs)" % (time() - t0))
