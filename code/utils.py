...
def my_collate(batch, sort):
    ...
    x, y, _ = pad(x, y, opt.eos_idx, sort)
...
def fakelabel_data_sorted_collate(batch):
    return fakelabel_data_collate(batch, sort=True)


def fakelabel_data_unsorted_collate(batch):
    return fakelabel_data_collate(batch, sort=False)


def fakelabel_data_collate(batch, sort):
    data, label, pred_value = zip(*batch)
    # extract input indices
    data = [s['inputs'] for s in data]
    # 明天好好研究一下这个函数
    data, label, pred_value = pad(data, label, opt.eos_idx, sort, pred_value)
    if torch.cuda.is_available():
        data = (data[0].cuda(0), data[1].cuda(0))
        label = label.cuda(0)
        pred_value = pred_value.cuda(0)
    return data, label, pred_value


def pad(x, y, eos_idx, sort, pred_value=None):
    ...
    if pred_value is not None:
        pred_value = torch.FloatTensor(pred_value).view(-1)
    if sort:
        # sort by length
        ...
        if pred_value is not None:
            pred_value = pred_value.index_select(0, sort_idx)
        return (padded_x, sort_len), y, pred_value
    else:
        return (padded_x, lengths), y, pred_value
