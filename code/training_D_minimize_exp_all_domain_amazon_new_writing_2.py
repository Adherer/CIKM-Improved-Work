...
def train(train_sets, dev_sets, test_sets, unlabeled_sets):
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i in tqdm(range(num_iter)):
            ...
            lambda1 = 1
            lambda2 = 1
            source_moving_centroid = 0.
            target_moving_centroid = 0.
            # D iterations
            ...
            for domain in opt.domains:
                ...
                loss_part_3, source_centroid, target_centroid = \
                    center_point_constraint(domain_feat, targets,
                                            source_moving_centroid, target_moving_centroid)
                loss_part_3 = lambda2 * loss_part_3
                source_moving_centroid = source_centroid
                target_moving_centroid = target_centroid
...
def center_point_constraint(F_d_features, targets, source_moving_centroid, target_moving_centroid):

    F_d_pre_half_features = F_d_features[:F_d_features.shape[0] // 2].cpu()
    F_d_suf_half_features = F_d_features[F_d_features.shape[0] // 2:].cpu()

    pre_half_samples_label = targets[:targets.shape[0] // 2].squeeze().cpu()
    suf_half_samples_label = targets[targets.shape[0] // 2:].squeeze().cpu()

    print(F_d_pre_half_features.shape)
    print(F_d_suf_half_features.shape)
    print(pre_half_samples_label)
    print(suf_half_samples_label)

    current_pre_half_samples_ones = torch.ones(F_d_pre_half_features.shape[0])
    current_suf_half_samples_ones = torch.ones(F_d_suf_half_features.shape[0])

    current_pre_half_samples_count = unsorted_segment_sum(current_pre_half_samples_ones, pre_half_samples_label, 2)
    current_suf_half_samples_count = unsorted_segment_sum(current_suf_half_samples_ones, suf_half_samples_label, 2)

    print("==============================")
    print(current_pre_half_samples_count)
    print(current_suf_half_samples_count)

    current_positive_pre_half_samples_count = torch.max(current_pre_half_samples_count,
                                                        torch.ones_like(current_pre_half_samples_count))
    current_positive_suf_half_samples_count = torch.max(current_suf_half_samples_count,
                                                        torch.ones_like(current_suf_half_samples_count))

    print("------------------------------")
    print(current_positive_pre_half_samples_count)
    print(current_positive_suf_half_samples_count)

    current_pre_half_samples_centroid = torch.div(
        unsorted_segment_sum(data=F_d_pre_half_features, segment_ids=pre_half_samples_label, num_segments=2),
        current_positive_pre_half_samples_count[:, None]).to(opt.device)
    current_suf_half_samples_centroid = torch.div(
        unsorted_segment_sum(data=F_d_suf_half_features, segment_ids=suf_half_samples_label, num_segments=2),
        current_positive_suf_half_samples_count[:, None]).to(opt.device)

    print("*****************************")
    print(current_pre_half_samples_centroid)
    print(current_suf_half_samples_centroid)

    source_centroid = 0.3 * current_pre_half_samples_centroid + 0.7 * source_moving_centroid
    target_centroid = 0.3 * current_suf_half_samples_centroid + 0.7 * target_moving_centroid

    center_loss = torch.mean(torch.pow(source_centroid - target_centroid, 2))
    return center_loss, source_centroid, target_centroid
