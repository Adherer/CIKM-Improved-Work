...
# ---------------------- no F_d_target写法的简单思路 ---------------------- #
# 1.首先，重头戏还是在于D的训练，但是由于训练数据有限，所以可能会利用这一批数据，训练多个epoch；
#   还有一种思路就是不按照MAN模型的思路来，采用GRL梯度反转层，并且对D的网络结构进行一定的设计，当然这时也应当物色一些新的数据集
# 2.由于这里不再训练F_d_target了，因此也就不需要生成伪标签，所以genarate_labels函数可以有一定的改动(训练D也就更灵活，为了测试D的收敛性
#   可以另外写一个函数，来检验生成标签的正确率大概是多少)
# 2.1: 但是这里需要注意的是，这里需要把经过F_d和F_s的feature表示整合成一个"新的"数据集
# 2.2: 需要注意的是，在测试阶段仍然需要generate_label，用以验证模型预测的正确率
# 3.等师兄来了，让师兄讲一下ICLR 2019那一篇文章的训练思路，看是否可以套用
