...
fh = logging.FileHandler(os.path.join(opt.exp2_target_model_save_file, '2019.08.05_exp2_log.txt'))
log.addHandler(fh)
...
# ---------------------- training ---------------------- #
def train(train_sets, test_sets, margin, lambda1, lambda2):
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i in tqdm(range(num_iter)):
            ...
            for domain in opt.domains:
                ...
                loss_part_2 = lambda1 * margin_regularization(inputs, targets_onehot, F_d[domain], margin)
        ...
        # end of epoch
        print('Ending epoch {}'.format(epoch+1))
        if d_total > 0:
            print('D Training Accuracy: {}%'.format(100.0*d_correct/d_total))
        print('Training accuracy:')
        print('\t'.join(opt.domains))
        print('\t'.join([str(100.0*correct[d]/total[d]) for d in opt.domains]))
    ...
    if avg_test_acc > best_acc:
        print(f'New best Average test accuracy: {avg_test_acc}')
...
def margin_regularization(inputs, targets, F_d, margin):
    features = F_d(inputs)
...
def evaluate(name, loader, F_d, C):
    ...
    acc = correct / total
    print('{}: Accuracy on {} samples: {}%'.format(name, total, 100.0*acc))
    return acc
...
def main():
    ...
    ave_acc = 0.0
    margin_lower, margin_upper = 3, 30
    lambda1_lower, lambda1_upper = 1, 10
    lambda2_lower, lambda2_upper = 1, 100
    for margin in range(margin_lower, margin_upper):
        for lambda1 in range(lambda1_lower, lambda1_upper):
            lambda1 = lambda1 / 100
            for lambda2 in range(lambda2_lower, lambda2_upper):
                lambda2 = lambda2 / 100
                for u_domain in unlabeled_domains:
                    ...
                    best_acc, best_acc_dict, visual_features, senti_labels = \
                        train(train_sets, test_sets, margin, lambda1, lambda2)

                    log.info("margin: " + str(margin))
                    log.info("lambda1: " + str(lambda1))
                    log.info("lambda2: " + str(lambda2))
